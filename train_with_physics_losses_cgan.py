...
from generator_model_cgan import MultiSpectralPatchToProspectGenerator

from discriminator_model_cgan import (
    SegmentedSpectralDiscriminator1D,
    ConditionalSegmentedSpectralDiscriminator1D,
)
...
def permute_batch_bands(batch_bands, perm):
    """
    Reorder the image-patch conditions across the batch.

    Used for paired cGAN mismatch training:
        D(real_spectrum_i, image_condition_j) -> fake, j != i
    """
    ...
    for idx, (batch_bands, y_real) in enumerate(loop):
        ...
        with torch.amp.autocast("cuda", enabled=use_amp):
            if use_conditional_discriminator:
                D_real = disc(y_real, batch_bands)
                D_fake = disc(y_fake.detach(), batch_bands)
            else:
                D_real = disc(y_real)
            ...
            if use_conditional_discriminator and float(lambda_mismatch) > 0.0:
                ...
                if perm is not None:
                    mismatched_bands = permute_batch_bands(batch_bands, perm)
        ...
        with torch.amp.autocast("cuda", enabled=use_amp):
            if use_conditional_discriminator:
                D_fake_for_G = disc(y_fake, batch_bands)
            else:
...
def main():
    ...
    if use_conditional_discriminator:
        disc = ConditionalSegmentedSpectralDiscriminator1D(
            **discriminator_common_kwargs,
            condition_dim=getattr(
                config, "CONDITION_DIM", config.DISCRIMINATOR_FEATURES[-1]
            ),
            condition_embed_dim=getattr(
                config, "CONDITION_EMBED_DIM", config.EMBED_DIM
            ),
            condition_base_features=getattr(
                config, "CONDITION_BASE_FEATURES", config.BASE_FEATURES
            ),
            condition_patch_encoder_type=getattr(
                config, "CONDITION_PATCH_ENCODER_TYPE", config.PATCH_ENCODER_TYPE
            ),
            condition_pooling_type=getattr(
                config, "CONDITION_POOLING_TYPE", config.POOLING_TYPE
            ),
            condition_band_encoder_mode=getattr(
                config, "CONDITION_BAND_ENCODER_MODE", config.BAND_ENCODER_MODE
            ),
            condition_norm_type=getattr(
                config, "CONDITION_NORM_TYPE", config.NORM_TYPE
            ),
        ).to(device)
    else:
    ...
    physics_loss_fn = PhysicsInformedLoss(
        ...
        lambda_weighted=config.LAMBDA_WEIGHTED,
        lambda_param_penalty=config.LAMBDA_PARAM_PENALTY,
        lambda_smoothness=config.LAMBDA_SMOOTHNESS,
        lambda_derivative=config.LAMBDA_DERIVATIVE,
        lambda_segment_continuity=getattr(config, "LAMBDA_SEGMENT_CONTINUITY", 0.5),
    ...
    print(f"patch_w: {config.PATCH_W}")
    print(f"patch_h: {config.PATCH_H}")
    print(f"PROSPECT_BOUNDS: {config.PROSPECT_PARAM_MAXS}")
    ...
    print(f"  Generator SEGMENTS:             {gen.spectral_segments}")
