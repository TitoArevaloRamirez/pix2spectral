...
def infer_groupA_generator_flags(model_spec: Dict[str, str]) -> Tuple[bool, bool]:
    """
    Infer generator architecture from Group A variant label/folder.

    Returns:
        use_segmented_prospect, use_segment_residual
    ...
    This is required because the checkpoint tensor shapes differ:
        full PROSPECT      -> param_mlp output 7
        segmented PROSPECT -> param_mlp output 4 * 7 = 28
        residual=True      -> residual_mlp exists
        residual=False     -> residual_mlp does not exist
    """
    text = (
        str(model_spec.get("mode_label", ""))
        + " "
        + str(model_spec.get("experiment_dir", ""))
        + " "
        + str(model_spec.get("experiment_path", ""))
    ).lower()

    if (
        "g1" in text
        or "full_prospect_no_residual" in text
        or "full_no_residual" in text
    ):
        return False, False

    if "g2" in text or "full_prospect_residual" in text or "full_residual" in text:
        return False, True

    if (
        "g3" in text
        or "segmented_prospect_no_residual" in text
        or "segmented_no_residual" in text
    ):
        return True, False

    if (
        "g4" in text
        or "segmented_prospect_residual" in text
        or "segmented_residual" in text
    ):
        return True, True

    # Fallback for non-Group-A use. These defaults match the current improved
    # generator configuration and can still be overridden by config before calling
    # apply_groupA_generator_flags_to_cfg.
    return True, True


def apply_groupA_generator_flags_to_cfg(
    cfg, model_spec: Dict[str, str]
) -> Tuple[bool, bool]:
    """
    Mutate cfg before building the generator so the model architecture matches
    the checkpoint for the current Group A variant.
    """
    use_segmented, use_residual = infer_groupA_generator_flags(model_spec)

    setattr(cfg, "USE_SEGMENTED_PROSPECT", bool(use_segmented))
    setattr(cfg, "USE_SEGMENT_RESIDUAL", bool(use_residual))

    # Segment continuity is meaningful only for segmented generators.
    if not use_segmented and hasattr(cfg, "LAMBDA_SEGMENT_CONTINUITY"):
        setattr(cfg, "LAMBDA_SEGMENT_CONTINUITY", 0.0)

    print(
        "Generator architecture for checkpoint loading: "
        f"use_segmented_prospect={use_segmented}, "
        f"use_segment_residual={use_residual}"
    )

    return use_segmented, use_residual
...
def evaluate_model_on_stage(
    ...
    # IMPORTANT for Group A:
    # Each variant has a different generator architecture. Apply flags before
    # constructing the generator, otherwise checkpoint shapes will mismatch.
    apply_groupA_generator_flags_to_cfg(cfg, model_spec)
...
def make_mean_std_tables(metrics_df: pd.DataFrame, output_dir: Path):
    ...
    for (mode, stage), g in metrics_df.groupby(group_cols):
        ...
        for metric in primary_metrics + optional_metrics:
            ...
            row[f"{metric}_std"] = (
                round(float(g[metric].std(ddof=1)), 3) if len(g) > 1 else 0.0
            )
            # row[f"{metric}_mean_std"] = (
            #    f"{row[f'{metric}_mean']:.6g} +/- {row[f'{metric}_std']:.6g}"
            # )
        rows.append(row)
