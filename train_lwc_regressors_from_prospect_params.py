...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--grid-size", choices=["small", "medium", "wide"], default="medium")
    parser.add_argument(
        "--grid-refit-metric",
        choices=["rmse", "mae", "r2"],
        default="rmse",
...
def prepare_dataframe(csv_path: str, args: argparse.Namespace, split_name: str) -> pd.DataFrame:
    ...
    if df[args.target_column].isna().any():
        ...
        if args.drop_na_target:
            print(f"Warning: dropping {n_bad} rows with missing {args.target_column} in {split_name}.")
...
def add_or_infer_groups(df: pd.DataFrame, group_column: str) -> Tuple[Optional[np.ndarray], str]:
...
def get_hyperparameters(species: str, grape_gb_n_estimators: int) -> Dict[str, Dict[str, Any]]:
...
def make_models(
    ...
    if use_cuml:
        try:
            ...
            print(f"cuML estimator construction failed; falling back to CPU. Reason: {exc}")
            use_cuml = False
    ...
    if use_xgb:
        try:
            ...
            print(f"XGBoost GPU estimator construction failed; falling back to sklearn GB. Reason: {exc}")
            use_xgb = False
...
def make_param_grids(
    ...
    if grid_size == "small":
        return {
            ...
            "Random Forest": {
                "n_estimators": sorted(set([hp["Random Forest"]["n_estimators"], 300, 600])),
            ...
            "Gradient Boosting": {
                "learning_rate": sorted(set([hp["Gradient Boosting"]["learning_rate"], 0.03, 0.05])),
                "max_depth": sorted(set([hp["Gradient Boosting"]["max_depth"], 2, 3])),
                "n_estimators": sorted(set([hp["Gradient Boosting"]["n_estimators"], 200, 400])),
                "subsample": sorted(set([hp["Gradient Boosting"]["subsample"], 0.8, 1.0])),
            },
        }
...
def make_cv_splitter_and_groups(
    ...
    return KFold(n_splits=n_splits, shuffle=True, random_state=random_state), None, "KFold"
...
def run_grid_search_for_model(
    ...
    results_path = output_dir / f"grid_search_results_{safe_model_filename(model_name)}.csv"
...
def run_model_evaluation(
    ...
    cv, cv_groups, cv_name = make_cv_splitter_and_groups(groups_dev, n_splits, random_state)
    ...
    for model_name in MODEL_ORDER:
        ...
        cv_pred_df["y_pred_normalized"] = np.asarray(y_dev_pred_norm, dtype=float).reshape(-1)
        ...
        plot_scatter(
            ...
            out_path=scatter_cv_dir / f"cv_scatter_{safe_model_filename(model_name)}.svg",
            metrics=cv_metrics,
        )
        ...
        test_pred_df["y_pred_normalized"] = np.asarray(y_test_pred_norm, dtype=float).reshape(-1)
        ...
        plot_scatter(
            ...
            eval_name="Test set",
            target_name=target_column,
            out_path=scatter_test_dir / f"test_scatter_{safe_model_filename(model_name)}.svg",
    ...
    cv_metrics_df = pd.DataFrame(cv_metric_rows).set_index("Model").loc[MODEL_ORDER].reset_index()
    test_metrics_df = pd.DataFrame(test_metric_rows).set_index("Model").loc[MODEL_ORDER].reset_index()
    ...
    if save_models:
        ...
        joblib.dump(fitted_models, model_dir / "sklearn_regressors_normalized_input.joblib")
        for model_name, estimator in fitted_models.items():
            joblib.dump(estimator, model_dir / f"{safe_model_filename(model_name)}_normalized_input.joblib")
...
def save_metrics_tables(
    ...
    (output_dir / "test_metrics.md").write_text(test_metrics_df.to_markdown(index=False))
...
def main() -> int:
    ...
    if args.normalization_scope == "all_leaf_samples":
        print("Normalization will be fitted using all leaf samples: train + validation + test.")
    ...
    param_shape_counts = pd.Series([str(s) for s in param_shapes_dev + param_shapes_test]).value_counts()
    ...
    print(f"  Target min/max dev:  {np.min(y_dev_original):.6f} / {np.max(y_dev_original):.6f}")
    print(f"  Target min/max test: {np.min(y_test_original):.6f} / {np.max(y_test_original):.6f}")
    print(f"  Normalized y dev:    {np.min(y_dev_norm):.6f} / {np.max(y_dev_norm):.6f}")
    print(f"  Normalized y test:   {np.min(y_test_norm):.6f} / {np.max(y_test_norm):.6f}")
    ...
    if args.save_models:
        ...
        joblib.dump(full_artifact, model_dir / "prospect_param_regressors_with_global_normalizers.joblib")
    ...
    manifest = {
        ...
        "outputs": {
            ...
            "selected_best_hyperparameters": str(output_dir / "selected_best_hyperparameters.csv"),
            "prospect_parameter_features": str(output_dir / "prospect_parameter_features.csv"),
            ...
            "models": str(output_dir / "fitted_models_joblib") if args.save_models else None,
        },
    }
    ...
    print("")
    print(f"Feature list:                {output_dir / 'prospect_parameter_features.csv'}")
    ...
    if args.save_models:
        ...
        print(f"Global normalizer artifact:  {output_dir / 'fitted_models_joblib' / 'prospect_param_regressors_with_global_normalizers.joblib'}")
    print("=" * 80)
