...
def evaluate_one_stage(
    ...
    if stats_stage not in normalization_stats_cache:
        print(f"Computing normalization stats from TRAIN set with stage='{stats_stage}'...")
    ...
    with torch.no_grad():
        for local_idx, (batch_bands, y_real) in enumerate(loader):
            ...
            if not np.isfinite(y_real_np).all():
                raise FloatingPointError(f"Non-finite y_real at stage={stage}, index={local_idx}")
            if not np.isfinite(y_fake_np).all():
                raise FloatingPointError(f"Non-finite y_fake at stage={stage}, index={local_idx}")
            ...
            if hasattr(test_dataset, "df"):
                ...
                for col in ["Species", "Stages", "blue", "green", "red", "nir", "red_edge"]:
                    if col in df_row:
...
def parse_args():
    ...
    parser.add_argument("--results-dir", default="~/Results/pix2spectral")
...
def main():
    ...
    if args.img_dir is None:
        args.img_dir = get_cfg_value(cfg, ["TEST_IMG_DIR", "VAL_IMG_DIR", "TRAIN_IMG_DIR"], None)
    ...
    if args.img_dir is None:
        raise ValueError("No --img-dir provided and config has no TEST_IMG_DIR/VAL_IMG_DIR/TRAIN_IMG_DIR.")
    ...
    if args.device is None:
        requested = get_cfg_value(cfg, ["DEVICE"], "cuda" if torch.cuda.is_available() else "cpu")
    else:
    ...
    if not all_rows:
        raise RuntimeError("No evaluation rows were produced. Check test CSV/stage filters.")
