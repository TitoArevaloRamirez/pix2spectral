...
def save_checkpoint(
    ...
    Includes extra metadata so the checkpoint can be used both for:
      - resuming training
      - preserving the best validation model
    """
    ckpt = {
...
def load_checkpoint(
    ...
    Backward-compatible with your old checkpoints that only contain:
...
def validate(gen, val_loader, device, use_amp, physics_loss_fn, sam_fn):
    ...
    Important:
        Validation does not update the discriminator.
        The selected best model should normally be based on one validation
        reconstruction/physics metric such as val_l1 or val_physics_total.
    """
    gen.eval()
    ...
    with torch.no_grad():
        for batch_bands, y_real in val_loader:
            ...
            with torch.amp.autocast("cuda", enabled=use_amp):
                ...
                rmse = torch.sqrt(torch.mean((y_fake - y_real) ** 2))
...
def main():
    ...
    if config.LOAD_MODEL:
        ...
        if os.path.isfile(gen_ckpt_path):
            ...
            print(f"✅ Loaded generator checkpoint: {gen_ckpt_path}")
        ...
        if os.path.isfile(disc_ckpt_path):
            ...
            print(f"✅ Loaded discriminator checkpoint: {disc_ckpt_path}")
    ...
    train_dataset = MultiSpectralCSVPatchDataset(
        ...
        min_leaf_coverage=config.LEAF_COVERAGE,  # 0.90,
        min_patches_per_band=config.MIN_PATCHES,  # 10,
        max_patches_per_band=None,
        border_erode_px=2,
        mask_method=config.MASK_METHOD,  # "contour",
        random_seed=config.RANDOM_SEED,  # 123,
        return_debug=False,
    )
    ...
    for epoch in range(start_epoch, config.NUM_EPOCHS):
        ...
        # Log metrics
        log_metrics(
            epoch, all_metrics, getattr(config, "LOG_FILE", "training_log.json")
        )
        ...
        print(
            ...
            f"({val_metrics['val_sam_rad']:.4f} rad)"
        )
        ...
        # Save best validation model immediately when improved.
        if config.SAVE_MODEL and improved:
            ...
            print(f"     Discriminator: {best_disc_path}")

        # Save validation visualizations only at requested interval or when best.
        should_plot = (
        ...
        # Save latest/resume checkpoints periodically.
    ...
    # ============================================================
    if config.SAVE_MODEL and os.path.isfile(best_gen_path):
        ...
        print(f"✅ Final model exported from best validation checkpoint:")
